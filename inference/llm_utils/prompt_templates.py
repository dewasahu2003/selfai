...
# getting similar/best questions - needed for retrieval
class QueryExpansionTemplate(BasePromptTemplate):
    ...
    @property
    def seprator(self) -> str:
        return "#next-question#"

    def create_template(self, to_expand_to_n) -> PromptTemplate:
        return PromptTemplate(
            template=self.prompt,
            input_variables=["question"],
            partial_variables={
                "separator": self.seprator,
                "to_expand_to_n": to_expand_to_n,
            },
        )


# getting particular aspect from question
...
# getting the best answer lined in list form
class RerankingTemplate(BasePromptTemplate):
    ...
    def create_template(self, keep_top_k: int) -> PromptTemplate:
        return PromptTemplate(
            ...
            partial_variables={"separator": self.seprator, "keep_top_k": keep_top_k},
        )


# promt for getting inference from the model
# can be directly from the model - model will answer by its own
# can be via rag - model will fetch related content from vectordb and answer accordingly
...
# evaluting the only model outputs - we will use other llm to eval our finetuneed model
...
# evaluating our rag system by using other llm
